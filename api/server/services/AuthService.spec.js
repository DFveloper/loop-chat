...
describe('registerUser', () => {
  ...
  it('runs trusted post-create hooks and allows AXIOM callers to force USER', async () => {
    countUsers.mockResolvedValue(0);
    const onCreated = jest.fn();

    const result = await registerUser(registrationPayload, { role: 'USER' }, { onCreated });

    expect(result).toEqual(expect.objectContaining({ status: 200, userCreated: true }));
    expect(createUser.mock.calls[0][0]).toEqual(expect.objectContaining({ role: 'USER' }));
    expect(onCreated).toHaveBeenCalledWith(expect.objectContaining({ _id: 'new-user-id' }));
  });

  it('rolls back group state and the created user when a post-create hook fails', async () => {
    const hookError = new Error('group assignment failed');
    const onCreated = jest.fn().mockRejectedValue(hookError);
    const onRollback = jest.fn();
    deleteUserById.mockResolvedValue({ deletedCount: 1 });

    const result = await registerUser(registrationPayload, {}, { onCreated, onRollback });

    expect(result).toEqual({ status: 500, message: 'Something went wrong' });
    expect(onRollback).toHaveBeenCalledWith(expect.objectContaining({ _id: 'new-user-id' }));
