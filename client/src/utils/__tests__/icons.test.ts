import {
  ...
  isImageURL,
  isSameOriginOrDataIcon,
  isSvgIcon,
  isThemeAwareBrandLogo,
} from '../icons';
...
describe('theme-aware brand logos', () => {
  it('recognizes Aeonthic and Lumen branding without matching substrings', () => {
    expect(isThemeAwareBrandLogo('/assets/Aeonthic/logo.svg')).toBe(true);
    expect(isThemeAwareBrandLogo('/assets/lumen-logo.png')).toBe(true);
    expect(isThemeAwareBrandLogo('/assets/volumetric.svg')).toBe(false);
  });

  it('inverts dark logos in dark mode and white logos in light mode', () => {
    expect(getThemeAwareBrandLogoClass('/assets/Aeonthic/logo.svg')).toBe('dark:invert');
    expect(getThemeAwareBrandLogoClass('/assets/Aeonthic/logo-white.svg')).toBe(
      'invert dark:invert-0',
    );
    expect(getThemeAwareBrandLogoClass('/assets/provider.svg')).toBe('');
  });
});
